...
int main () {
...

	{
		// use the alternative iterator on the file
	        // load up the table supplier table from the catalog
                ...
                int counter = 0;
                while (myIter->advance ()) {  // 实际是从table中选择了一个当前page，再创建一个pageIter,迭代里面的record
...

	{
		...
		MyDB_RecordPtr temp = supplierTable.getEmptyRecord (); // go get a RecordPtr
		MyDB_RecordIteratorPtr myIter = supplierTable[36].getIterator (temp);  // go get a RecordIteratorPtr at 37th page
